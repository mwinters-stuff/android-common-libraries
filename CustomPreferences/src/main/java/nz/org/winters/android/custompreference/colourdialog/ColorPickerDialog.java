...
public class ColorPickerDialog extends DialogFragment implements ColorPickerSwatch.OnColorSelectedListener
{
  ...
  public static ColorPickerDialog newInstance(Context context, int selectedColor)
  {
    String[] choices = context.getResources().getStringArray(R.array.default_color_choice_values);
    int[] mColorChoices = new int[choices.length];
    for (int i = 0; i < choices.length; i++)
    {
      mColorChoices[i] = Color.parseColor(choices[i]);
    }
    return newInstance(R.string.color_picker_default_title,mColorChoices, selectedColor, 5,
            Utils.isTablet(context)? ColorPickerDialog.SIZE_LARGE : ColorPickerDialog.SIZE_SMALL);
  }
